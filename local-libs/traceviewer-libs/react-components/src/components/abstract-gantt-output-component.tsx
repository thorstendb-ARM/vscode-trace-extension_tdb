...
export type AbstractGanttOutputState = AbstractTreeOutputState & {
    chartTree: TimeGraphEntry[];
...
export abstract class AbstractGanttOutputComponent<
    P extends AbstractGanttOutputProps = AbstractGanttOutputProps,
    S extends AbstractGanttOutputState = AbstractGanttOutputState
> extends AbstractTreeOutputComponent<P, S> {
    ...
    constructor(props: P) {
        ...
        this.chartTreeRef = React.createRef();
        ...
        this.COARSE_RESOLUTION_FACTOR = this.props.coarse_resolution_factor ?? 1;
        ...
        const providers: TimeGraphChartProviders = {
            ...
            ) => this.fetchChartData(range, resolution, fetchArrows, rowIds, additionalProperties),
        ...
        this.rowController.onVerticalOffsetChangedHandler(() => {
            if (this.chartTreeRef.current) {
                this.chartTreeRef.current.scrollTop = this.rowController.verticalOffset;
            }
        });
    ...
    synchronizeTreeScroll(): void {
        if (this.chartTreeRef.current) {
            this.rowController.verticalOffset = this.chartTreeRef.current.scrollTop;
        }
    }
    ...
    async componentDidUpdate(prevProps: AbstractGanttOutputProps, prevState: AbstractGanttOutputState): Promise<void> {
        if (
            ...
            if (
                ...
                !isEqual(this.state.chartTree, prevState.chartTree) ||
        ...
        if (this.state.chartTree.length > 0 && prevState.chartTree.length === 0) {
    ...
    protected onToggleAnnotationCollapse(): void {
    ...
    protected onMarkerCategoryRowClose(id: number): void {
    ...
    private isVisible(entry: TimeGraphEntry): boolean {
        ...
        while (parentId !== undefined && parentId !== -1) {
            ...
            const parent = this.state.chartTree.find(e => e.id === parentId);
    ...
    private doHandleOrderChange(ids: number[]) {
        const ordered = this.state.chartTree.slice().sort((a, b) => ids.indexOf(a.id) - ids.indexOf(b.id));
        this.setState({ chartTree: ordered });
    }

    private doHandleOrderReset() {
        this.doHandleOrderChange(this.state.defaultOrderedIds);
    }
    ...
    private findElement(payload: { [key: string]: string | { [key: string]: string } }): TimeGraphEntry | undefined {
        ...
        if (payload && payload.load) {
            this.state.chartTree.forEach(el => {
                if (el.metadata) {
                    let cnt = 0;
    ...
    protected getMarkersLayerHeight(): number {
        const rowHeight = 20;
    ...
    onCtxMenu(event: React.MouseEvent<HTMLDivElement>, id: number): void {
        ...
        const { show } = useContextMenu({
            id: `${this.MENU_ID}-${this.props.outputDescriptor.id}`
        });
    ...
    protected getEntryModelsForRowIds = (
        ...
        for (const id of ids) {
            const element = this.state.chartTree.find(el => el.id === id);
            if (element) {
    ...
    private renderAbstractGanttContent(id: string) {
        return (
    ...
    private getSearchBar() {
        return (
            <div id={this.props.traceId + this.props.outputDescriptor.id + 'searchBar'} className="gantt-search-bar">
                <TextField
                    InputProps={{
                        placeholder: 'Search',
                        startAdornment: (
    ...
    private expandParents(entry: TimeGraphEntry) {
        let foundNode = this.state.chartTree.find(node => node.id === entry?.id);
        if (foundNode) {
            let parentId: number | undefined = foundNode.parentId;
            const ids: number[] = [];
            while (parentId && parentId >= 0) {
                ids.push(parentId);
                foundNode = this.state.chartTree.find(node => node.id === parentId);
    ...
    public onMultipleRowClick = (id: number, isShiftClicked?: boolean): void => {
        const tree = listToTree(this.state.chartTree, this.state.columns);
